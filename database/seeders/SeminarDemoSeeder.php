...
/**
 * DATA DEMO SEMINAR - BUKAN UNTUK PRODUCTION.
 *
 * Dataset untuk demonstrasi live seminar kerja praktik. Adaptasi langsung
 * dari DocumentationSeeder (dataset screenshot Buku Panduan Pengguna) -
 * SELURUH isinya tetap fiktif: tidak ada nama client nyata, email/nomor
 * telepon orang nyata, API token, maupun URL posting media sosial nyata.
 * Lihat docs/SEEDER_INVENTORY.md §4 untuk alasan kenapa seeder ini file
 * terpisah, bukan memodifikasi DocumentationSeeder yang sudah ada.
 *
 * BEDA UTAMA dari DocumentationSeeder (kenapa tidak dipakai apa adanya):
 * 1. Delay Risk BENAR-BENAR dihitung oleh DelayRiskPredictionService (model
 *    ML Python asli) lewat computeDelayRiskScores() di akhir run(), BUKAN
 *    ditulis sebagai angka lepas - lihat docblock method itu. Konsekuensi:
 *    seeder ini WAJIB dijalankan di environment yang python+model-nya
 *    sudah tersedia (persis kondisi environment demo seminar - aplikasi
 *    memang berjalan penuh saat itu), beda dari DocumentationSeeder yang
 *    sengaja dirancang TIDAK bergantung Python sama sekali.
 * 2. Menambah seedMetricSnapshots() (ContentMetricSnapshot) yang tidak ada
 *    di DocumentationSeeder - dibutuhkan supaya Bonus Performa KPI (baca
 *    D+7..D+10 dari tabel ini, lihat TeamPerformanceKpiCalculator) benar-
 *    benar bisa dihitung, bukan selalu "-" (structural limitation A di
 *    docs/PRE_SEMINAR_SYSTEM_AUDIT.md §7).
 * 3. Akun CEO memakai email Gmail ASLI dari config('seminar_demo.login_email')
 *    (env SEMINAR_DEMO_LOGIN_EMAIL) - supaya presenter bisa login live lewat
 *    Google OAuth sungguhan saat seminar (login sistem TIDAK auto-provision
 *    user dan TIDAK punya password login, murni lookup email yang sudah
 *    ada - lihat GoogleAuthController). guardEnvironment() menolak jalan
 *    kalau env ini belum diisi.
 * 4. Bootstrap role/permission dipanggil SENDIRI oleh seeder ini
 *    (PermissionSeeder::class) - TIDAK menyuruh pengguna menjalankan
 *    `php artisan db:seed` dulu, karena itu akan memanggil TeamClientSeeder
 *    (data client/staf 523 Studio ASLI) yang justru harus dihindari di
 *    database demo seminar.
 *
 * Jalankan eksplisit (TIDAK pernah dipanggil DatabaseSeeder):
 *   php artisan migrate:fresh
 *   SEMINAR_DEMO_LOGIN_EMAIL=alamat.anda@gmail.com php artisan db:seed --class=SeminarDemoSeeder
 * (atau isi SEMINAR_DEMO_LOGIN_EMAIL di .env lebih dulu, lalu cukup
 * `php artisan db:seed --class=SeminarDemoSeeder`)
 *
 * CATATAN DESAIN YANG TETAP DIPERTAHANKAN dari DocumentationSeeder:
 ...
 *    "Terhubung" padahal tidak. Kalau mau mendemokan kondisi "Terhubung",
 *    itu HARUS dari akun tester sungguhan, bukan data seeder.
 ...
 * 3. Semua angka RAW (views, follower, dst) pakai PRNG deterministik
 *    internal (bukan rand()/mt_rand()), jadi dua kali seed menghasilkan
 *    input yang sama - hanya Delay Risk & KPI yang genuinely dihitung ulang
 *    tiap run (nilai KEDUANYA tetap deterministik SELAMA model ML & kode
 *    kalkulatornya tidak berubah, karena inputnya deterministik).
 * 4. Client & User memakai identitas fiktif YANG SAMA dengan
 *    DocumentationSeeder (sudah divalidasi aman dipublikasikan) - JANGAN
 *    jalankan dua seeder ini di database yang sama, karena Client/User
 *    di-keyed by name/email dan akan saling menimpa satu sama lain.
 */
class SeminarDemoSeeder extends Seeder
{
    /**
     ...
     */
    public const MARKER = 'seminar_demo_seeder';
    ...
    public function run(): void
    {
        ...
        $this->command?->warn('SEMINAR DEMO DATA - NOT FOR PRODUCTION');
        $this->command?->info('Seluruh data yang dibuat seeder ini fiktif dan hanya untuk demonstrasi seminar kerja praktik.');

        // Role & permission dibuat sendiri di sini (bukan lewat
        // `php artisan db:seed` biasa) - lihat docblock kelas ini poin 4.
        ...
        $this->seedMetrics();
        // Bonus Performa (D+7..D+10) - dibaca dari ContentMetric yang baru
        // saja dibuat seedMetrics(), lihat docblock method-nya.
        $this->seedMetricSnapshots();
        $this->seedAudience();
        $this->seedSyncLogs();
        ...
        $this->seedPins();
        // Delay Risk BARU dihitung setelah SEMUA data workflow/revisi/status
        // log di atas ada - fitur modelnya (workload PIC, revision_count,
        // days_in_current_status) dibaca live dari database saat ini
        // dipanggil, lihat docblock method-nya.
        $this->computeDelayRiskScores();
     ...
     * "production"/"prod" hampir pasti bukan tempat data fiktif), ditambah
     * satu guard khusus seeder ini: email login demo WAJIB sudah diisi,
     * supaya presenter tidak baru sadar tidak bisa login saat sudah di
     * depan panel.
     */
    private function guardEnvironment(): void
    {
        if (app()->isProduction()) {
            throw new \RuntimeException(
                'SeminarDemoSeeder menolak berjalan di environment production. '
                .'Seeder ini membuat client/user/konten FIKTIF untuk demonstrasi seminar.'
            );
        }
        ...
        if (preg_match('/(^|[_-])(prod|production|live)([_-]|$)/i', $database)) {
            throw new \RuntimeException(
                "SeminarDemoSeeder menolak berjalan di database \"{$database}\" - namanya terbaca sebagai database production."
            );
        }

        $loginEmail = config('seminar_demo.login_email');
        if (! $loginEmail || ! filter_var($loginEmail, FILTER_VALIDATE_EMAIL)) {
            throw new \RuntimeException(
                'SEMINAR_DEMO_LOGIN_EMAIL belum diisi di .env (atau formatnya bukan email valid). '
                .'Isi dengan alamat Gmail ASLI yang akan dipakai login live saat seminar - '
                .'login sistem murni Google OAuth lookup-by-email, jadi tanpa ini presenter '
                .'tidak akan bisa login sama sekali ke data yang dibuat seeder ini.'
            );
        }
    }
    ...
    private function purgePreviousRun(): void
    {
        ...
        if (! empty($itemIds)) {
            ...
            ContentMetricSnapshot::whereIn('content_item_id', $itemIds)->delete();
            ...
            // beda dari content item bernomor SEMINAR:* yang sengaja
            ...
            ContentItem::withTrashed()
                ->where('import_source', self::MARKER)
                ->where('external_reference', 'like', 'SEMINAR:slot:%')
                ->forceDelete();
        }
    ...
    {
        // Role & permission sudah dipasang di awal run() lewat
        // PermissionSeeder::class - cek ini murni jaring pengaman kalau
        // metode ini sampai dipanggil di luar urutan run() normal.
        ...
        if (! empty($missing)) {
            throw new \RuntimeException(
                'Role belum tersedia ('.implode(', ', $missing).'). PermissionSeeder seharusnya sudah membuatnya di awal run() - '
                .'cek apakah seedUsers() dipanggil di luar urutan run() normal.'
            );
        }
        ...
        // dashboard" - salah satu state yang layak ditunjukkan saat demo.
        //
        // CEO memakai email GMAIL ASLI dari config('seminar_demo.login_email')
        // (bukan @example.test seperti user lain) - itulah akun yang dipakai
        // presenter login live lewat Google OAuth, lihat guardEnvironment().
        $defs = [
            ['key' => 'ceo',        'name' => 'Ardi Pratama',  'roles' => ['CEO'],                        'login' => true, 'email' => config('seminar_demo.login_email')],
        ...
        foreach ($defs as $def) {
            ...
            $email = $def['email'] ?? ($slug.self::EMAIL_DOMAIN);
            ...
            $user = User::updateOrCreate(
                ...
                [
                    ...
                    'source' => self::MARKER,
                    'external_reference' => 'SEMINAR:user:'.$def['key'],
                ]
            );

            // Role diambil dari PermissionSeeder (tidak pernah membuat role
            // baru) - syncWithoutDetaching supaya role lain yang mungkin
            // diberikan manual saat uji coba tidak ikut terhapus.
    ...
    private function itemDefinitions(): array
    {
        return [
            ...
            // ---------- Kopi Senja: baseline Bonus Performa ----------
            // 3 konten TAMBAHAN di grup client+platform+format yang SAMA
            // dengan KS-02/KS-06 (Instagram, Carousel Feed) - Bonus Performa
            // KPI (TeamPerformanceKpiCalculator::analyticsBonusForItem())
            // butuh MINIMAL 3 publication pembanding sejenis yang tayang
            // LEBIH DULU sebelum bonus bisa dihitung sama sekali. Tanpa
            // ketiga baris ini, grup manapun di dataset curated hanya punya
            // 1-2 anggota - Bonus Performa akan SELALU "-" untuk semua
            // orang, bukan karena fitur rusak, tapi karena tidak ada cukup
            // pembanding. KS-B3 (paling baru, offset -45) sengaja diberi
            // performa jelas di atas rata-rata KS-B1/KS-B2/KS-02 di
            // seedMetrics() - itulah yang bakal tampil sebagai bonus positif
            // saat presenter membuka Performa Tim bulan itemnya tayang.
            ['ref' => 'KS-B1', 'client' => 'kopi', 'title' => 'Kombinasi Rasa Kopi dan Camilan Sore',  'type' => 'Desain', 'format' => 'Carousel Feed', 'platform' => 'Instagram', 'pillar' => 'Product Highlight','status' => 'uploaded',       'offset' => -95, 'pic' => 'designer',  'extra' => []],
            ['ref' => 'KS-B2', 'client' => 'kopi', 'title' => 'Cerita Pelanggan Pertama Kali Mampir',  'type' => 'Desain', 'format' => 'Carousel Feed', 'platform' => 'Instagram', 'pillar' => 'Soft Selling',     'status' => 'uploaded',       'offset' => -80, 'pic' => 'designer2', 'extra' => []],
            ['ref' => 'KS-B3', 'client' => 'kopi', 'title' => 'Sesi Latte Art Bareng Barista',         'type' => 'Desain', 'format' => 'Carousel Feed', 'platform' => 'Instagram', 'pillar' => 'Entertainment',    'status' => 'uploaded',       'offset' => -45, 'pic' => 'designer',  'extra' => ['smo' => 'smo']],
    ...
    private function seedContentItems(): void
    {
        foreach ($this->itemDefinitions() as $def) {
            ...
            $item = ContentItem::updateOrCreate(
                ['import_source' => self::MARKER, 'external_reference' => 'SEMINAR:'.$def['ref']],
                [
                    'content_plan_id' => $plan->id,
                    'client_id' => $client->id,
        ...
        foreach ($defs as $def) {
            ...
            foreach ($slots as $index => $slot) {
                $slot->forceFill([
                    ...
                    'external_reference' => 'SEMINAR:slot:'.$def['client'].':'.$slot->provisional_code,
    ...
    private function seedWorkflowFor(ContentItem $item, array $def): void
    {
        ...
        // withoutEvents(): ContentWorkflowObserver akan memanggil prediksi
        // Delay Risk SATU PER SATU tiap kali workflow berstatus brief_ready
        // dibuat - tidak efisien (satu proses Python per item) dan fitur
        // yang dibaca saat itu (workload PIC dkk) belum lengkap karena item
        // lain belum tentu selesai di-seed. Skor sungguhan dihitung SEKALI
        // di akhir run(), dalam batch, lewat computeDelayRiskScores() -
        // setelah SEMUA data sudah lengkap.
    ...
    /**
     * BEDA UTAMA dari DocumentationSeeder: skor TIDAK ditulis manual sama
     * sekali. Method ini memanggil DelayRiskPredictionService::predictForItems()
     * ASLI - persis service yang dipakai ContentWorkflowObserver di runtime
     * dan command `workflow:recompute-delay-risk` - dengan payload yang
     * dibangun dari fitur LIVE (workload PIC, revision_count,
     * days_in_current_status, dst.) hasil query ke data yang baru saja
     * ditulis seeder ini. Kalau model/script Python tidak tersedia di
     * environment ini, service-nya sendiri yang log+skip per item
     * (lihat DelayRiskPredictionService::callPredictScript()) - method ini
     * TIDAK pernah menulis skor sebagai fallback.
     *
     * Dipanggil SETELAH seluruh ContentWorkflow/ContentItemAssignment/
     * ContentRevision/ContentStatusLog selesai dibuat (run() memastikan
     * urutan ini) - kalau dipanggil lebih awal, fitur yang dibaca
     * (workload_pic_same_week dkk) akan mencerminkan data yang belum
     * lengkap.
     *
     * Scoping: hanya content item AKTIF (bukan draft/uploaded/cancelled -
     * sama seperti RecomputeDelayRiskScores di produksi) DAN milik seeder
     * ini (import_source = MARKER) yang diberi skor. Konsekuensi jujur:
     * konten yang sudah 'uploaded' di dataset ini TIDAK pernah punya skor
     * historis (beda dari DocumentationSeeder yang menulis skor historis
     * buatan khusus untuk itu) - kartu "Ketepatan Prediksi Risiko Tinggi"
     * kemungkinan akan tampil "belum ada cukup data" saat demo, karena
     * seeder ini sengaja tidak mensimulasikan histori prediksi sepanjang
     * siklus hidup tiap konten. Ini trade-off yang disengaja: skor yang
     * ADA harus 100% hasil model sungguhan, bukan mengorbankan itu demi
     * mengisi satu panel sekunder.
     */
    private function computeDelayRiskScores(): void
    {
        $itemIds = ContentWorkflow::whereHas('contentItem', fn ($q) => $q->where('import_source', self::MARKER))
            ->whereNotIn('current_status', WorkflowTransitions::INACTIVE_STATUSES)
            ->pluck('content_item_id')
            ->all();

        if (empty($itemIds)) {
            $this->command?->warn('Delay Risk: tidak ada content item aktif untuk dihitung.');

            return;
        }

        $service = app(DelayRiskPredictionService::class);
        $succeeded = 0;

        // Batch per 50 item, sama seperti RecomputeDelayRiskScores - satu
        // panggilan Python per chunk, bukan satu proses per item.
        ...
        if ($succeeded === 0) {
            $this->command?->warn(
                "Delay Risk: 0 dari {$requested} item aktif berhasil dapat skor. "
                .'Model (storage/ai/delay_risk/delay_risk_model.pkl) atau python tidak ditemukan di environment ini - '
                .'halaman Delay Risk akan tampil kosong sampai pipeline ML tersedia (cek PYTHON_BIN di .env kalau python ada tapi bukan di PATH default).'
            );
        } else {
            $this->command?->info("Delay Risk: {$succeeded} dari {$requested} item aktif berhasil dihitung SUNGGUHAN oleh model ML (bukan seeder).");
        }
    }
    ...
    private function seedMetrics(): void
    {
        $profiles = [
            ...
            // Baseline Bonus Performa (lihat itemDefinitions()) - KS-B1/KS-B2
            // sengaja moderat (dekat rata-rata grup), KS-B3 sengaja jelas di
            // atasnya supaya bonus yang terhitung tidak nol/mendekati nol.
            'KS-B1' => ['peak' => 2200,  'tau' => 4.5, 'er' => 2.8],
            'KS-B2' => ['peak' => 2600,  'tau' => 4.8, 'er' => 3.0],
            'KS-B3' => ['peak' => 6400,  'tau' => 5.5, 'er' => 5.2],
    ...
    // =================================================================
    // 12b. Snapshot metrik (Bonus Performa KPI) - BARU, tidak ada di
    // DocumentationSeeder
    // =================================================================

    /**
     * Mengisi content_metric_snapshots - tabel time-series TERPISAH dari
     * content_metrics, dipakai KHUSUS oleh
     * TeamPerformanceKpiCalculator::analyticsBonusForItem() untuk menghitung
     * Bonus Performa (observasi cumulative pada window D+7..D+10 setelah
     * tayang, lihat migration 2026_09_01_000001_create_content_metric_snapshots_table).
     * Tanpa tabel ini Bonus Performa akan SELALU "-" untuk semua orang di
     * seluruh dataset - persis structural limitation A yang dicatat di
     * docs/PRE_SEMINAR_SYSTEM_AUDIT.md §7 (dan alasan kenapa
     * DocumentationSeeder tidak pernah menunjukkan Bonus Performa terisi).
     *
     * Nilainya DIAMBIL dari ContentMetric yang baru saja dibuat seedMetrics()
     * (bukan angka baru yang lepas dari cerita performa di halaman lain) -
     * satu baris per publication, pada metric_date yang paling dekat dengan
     * published_at + 8 hari (tengah window D+7..D+10).
     *
     * Item yang publish-nya BELUM 7 hari lalu dilewati apa adanya - window
     * D+7 memang belum lewat, itu batasan wajar sistem yang sesungguhnya
     * (server produksi juga tidak bisa mengamati D+7 sebelum harinya
     * tiba), bukan bug seeder. Konsekuensinya: item 'this_month' yang baru
     * tayang beberapa hari sebelum seeder dijalankan TIDAK akan langsung
     * punya Bonus Performa - itemDefinitions() sudah menambahkan 3 konten
     * baseline (KS-B1/KS-B2/KS-B3) supaya minimal SATU bulan lampau tetap
     * bisa didemokan dengan bonus terisi.
     */
    private function seedMetricSnapshots(): void
    {
        $itemIds = ContentItem::where('import_source', self::MARKER)->pluck('id');

        $items = ContentItem::whereIn('id', $itemIds)->get()->keyBy('id');
        $publications = ContentPublication::whereIn('content_item_id', $itemIds)->get();

        $created = 0;

        foreach ($publications as $publication) {
            $item = $items->get($publication->content_item_id);
            if (! $item) {
                continue;
            }

            $publishedAt = Carbon::parse($publication->published_at);
            $windowStart = $publishedAt->copy()->addDays(7);
            $windowEnd = $publishedAt->copy()->addDays(10);

            if ($windowStart->gt($this->now)) {
                continue; // window D+7..D+10 belum lewat - lihat docblock.
            }

            $target = $publishedAt->copy()->addDays(8);

            $metric = ContentMetric::where('content_item_id', $item->id)
                ->whereBetween('metric_date', [$windowStart->toDateString(), $windowEnd->toDateString()])
                ->get()
                ->sortBy(fn ($m) => abs(Carbon::parse($m->metric_date)->diffInHours($target, false)))
                ->first();

            if (! $metric) {
                continue; // item ini tidak punya profil di $profiles seedMetrics() - dilewati apa adanya.
            }
            ...
            ContentMetricSnapshot::create([
                ...
                'saves' => $metric->saves,
                'profile_visit' => $metric->profile_visit,
                'engagement_rate' => $metric->engagement_rate,
                'watch_time_avg' => $metric->watch_time_avg,
                'completion_rate' => $metric->completion_rate,
            ]);
            $created++;
        }

        $this->command?->info("Bonus Performa: {$created} ContentMetricSnapshot dibuat (item yang window D+7..D+10-nya sudah lewat).");
    }
    ...
    private function report(): void
    {
        ...
        // cuma tidak masuk $this->items (tidak punya ref SEMINAR:<kode>).
        ...
        $lines = [
            ...
            'Metric snapshots   : '.ContentMetricSnapshot::whereIn('client_id', $clientIds)->count().' (dasar Bonus Performa KPI)',
            ...
            'Delay risk scores  : '.DelayRiskScore::whereIn('content_item_id', $itemIds)->count().' (dihitung SUNGGUHAN oleh model ML, lihat log di atas)',
        ...
        $this->command?->info('Login demo: buka /auth/google dan masuk dengan akun Gmail '.config('seminar_demo.login_email').' (login sebagai CEO "Ardi Pratama").');
    }
}
