...
<div x-data="{ editOperationalRole: null, assignClients: null, confirmDeactivate: null, confirmActivate: null, showCreateModal: {{ $errors->any() ? 'true' : 'false' }} }" class="p-4 sm:p-6 lg:p-8 max-w-[1400px] mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div>
            ...
            <p class="text-[var(--text-secondary)] text-sm mt-1">
                Roster anggota tim agensi - real dari Content Planner, lengkap dengan role operasional dan status akun sistem. Tidak semua anggota tim punya akun login.
            </p>
        </div>
    ...
    {{-- SATU tabel, TeamMember sebagai primary dataset, DUA aksi utama:
         "Edit Role" -> team_members.operational_role, "Assign Klien" ->
         team_member_client - keduanya selalu mengedit TeamMember yang
         sama dengan yang ditampilkan di row, terlepas dari apakah dia
         punya User terkait atau tidak (Langkah "Kelola Tim Actions fix").
         Halaman ini SENGAJA tidak lagi mengekspos users.roles sama sekali
         (Langkah "Final Simplifikasi - Kelola Tim") - dulu ada tombol
         "Kelola Akses Sistem"/"Edit Role Sistem" di sini, tapi 2 konsep
         role (operasional vs authorization dashboard) bikin UX
         membingungkan di 1 halaman yang sama. Authorization/permission
         User TETAP ada di backend (route/controller users.roles tidak
         dihapus), cuma entry point-nya di halaman ini yang dicabut.
         "Aktif/Nonaktif Akun" tetap murni status User, cuma muncul kalau
         TeamMember ini punya User terkait - TeamMember tanpa akun tidak
         punya aksi User apapun (tidak ada "Undang" per-baris - fitur baru
         yang tidak diminta). --}}
            ...
            <table class="w-full text-sm text-left">
                <thead class="bg-[var(--surface-page)]">
                    <tr class="text-[var(--text-muted)] text-[11px] uppercase tracking-wide">
                        ...
                        <th class="px-4 py-3 font-medium whitespace-nowrap">Klien Ditangani</th>
                ...
                <tbody>
                    @forelse ($teamMembers as $tm)
                        @php
                            $tmClients = $tm->clients->pluck('name');
                            $tmClientsLabel = $tmClients->isEmpty() ? '-' : $tmClients->take(3)->join(', ').($tmClients->count() > 3 ? ', +'.($tmClients->count() - 3).' lainnya' : '');
                            $user = $tm->user;
                        @endphp
                        <tr class="border-t border-[var(--surface-muted)] hover:bg-[var(--surface-page)] transition-colors">
                            <td class="px-6 py-3.5">
                                <div class="flex items-center gap-3">
                                    ...
                                    <p class="font-medium text-[var(--text-primary)]">{{ $tm->name }}</p>
                                </div>
                            </td>
                            <td class="px-4 py-3.5 text-[var(--text-secondary)] whitespace-nowrap">{{ $tm->email ?? '-' }}</td>
                            <td class="px-4 py-3.5 text-[var(--text-secondary)] whitespace-nowrap">{{ $tm->operational_role ?? '-' }}</td>
                            <td class="px-4 py-3.5 text-[var(--text-secondary)] max-w-[280px]">{{ $tmClientsLabel }}</td>
                            <td class="px-4 py-3.5">
                                @if ($tm->hasAccount())
                                    <span class="badge badge-success">Akun aktif</span>
                                @else
                                    <span class="badge badge-neutral">Belum memiliki akun</span>
                                @endif
                            </td>
                            <td class="px-4 py-3.5">
                                <div class="flex items-center justify-end gap-1">
                                    <button type="button" @click="editOperationalRole = {{ $tm->id }}"
                                    ...
                                    <button type="button" @click="assignClients = {{ $tm->id }}"
                        ...
                        {{-- Modal Edit Role (operational_role - TeamMember) --}}
                        <template x-teleport="body">
                            <div x-show="editOperationalRole === {{ $tm->id }}" x-cloak
                                 x-on:keydown.escape.window="editOperationalRole = null"
                                 class="fixed inset-0 z-50 flex items-center justify-center p-4" style="display: none;">
                                <div class="absolute inset-0 bg-[#14181a]/40" @click="editOperationalRole = null"></div>

                                <div x-show="editOperationalRole === {{ $tm->id }}" x-transition
                                     role="dialog" aria-modal="true" aria-labelledby="edit-role-modal-title-{{ $tm->id }}" x-trap="editOperationalRole === {{ $tm->id }}"
                                    ...
                                    <div class="flex items-center justify-between px-6 py-5 border-b border-[var(--border)]">
                                        <div>
                                            <h3 id="edit-role-modal-title-{{ $tm->id }}" class="font-display text-lg font-semibold text-[var(--text-primary)]">Edit Role</h3>
                                            <p class="text-xs text-[var(--text-muted)] mt-0.5">Jabatan operasional {{ $tm->name }} di agensi.</p>
                                        </div>
                                        <button type="button" @click="editOperationalRole = null" class="text-[var(--text-muted)] hover:text-[var(--text-secondary)]">
                                            <span class="material-symbols-outlined text-[19px]">close</span>
                                        </button>
                                    </div>

                                    <form action="{{ route('team-members.operational-role.update', $tm) }}" method="POST">
                                        @csrf @method('PUT')
                                        <div class="px-6 py-5">
                                            @if ($tm->user)
                                                <p class="text-xs text-[var(--text-muted)] bg-[var(--surface-page)] rounded-lg p-3 mb-3">Akses sistem (dashboard) {{ $tm->name }} akan ikut disesuaikan mengikuti role ini - satu role, bukan dua sistem terpisah.</p>
                                            @endif
                                            <p class="text-xs font-semibold text-[var(--text-muted)] uppercase mb-3">Pilih Role Operasional</p>

                                            <div class="space-y-2">
                                                @foreach (['CEO', 'Manager', 'Desain Grafis', 'SMO', 'Content Creator'] as $roleOption)
                                                    <label class="flex items-center gap-3 p-3 border border-[var(--border)] rounded-lg hover:bg-[var(--surface-page)] cursor-pointer">
                                                        <input type="radio" name="operational_role" value="{{ $roleOption }}"
                                                               {{ $tm->operational_role === $roleOption ? 'checked' : '' }}
                                                               class="border-[var(--border-strong)] text-[var(--brand)] focus:ring-[var(--brand)]">
                                                        <p class="text-sm font-medium text-[var(--text-primary)]">{{ $roleOption }}</p>
                                                    </label>
                                                @endforeach
                                            </div>
                                        </div>
                                        ...
                                        <div class="flex items-center gap-3 px-6 py-4 border-t border-[var(--border)]">
                                            ...
                                            <button type="button" @click="editOperationalRole = null" class="btn-secondary">
                                                Batal
                                            </button>
                                        </div>
                                    </form>
                        ...
                        {{-- Modal Assign Klien (team_member_client - TeamMember) --}}
                        <template x-teleport="body">
                            <div x-show="assignClients === {{ $tm->id }}" x-cloak
                                 x-on:keydown.escape.window="assignClients = null"
                                 class="fixed inset-0 z-50 flex items-center justify-center p-4" style="display: none;">
                                <div class="absolute inset-0 bg-[#14181a]/40" @click="assignClients = null"></div>

                                <div x-show="assignClients === {{ $tm->id }}" x-transition
                                     role="dialog" aria-modal="true" aria-labelledby="assign-client-modal-title-{{ $tm->id }}" x-trap="assignClients === {{ $tm->id }}"
                                     class="relative bg-[var(--surface-card)] rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
                                    <div class="flex items-center justify-between px-6 py-5 border-b border-[var(--border)]">
                                        <div>
                                            <h3 id="assign-client-modal-title-{{ $tm->id }}" class="font-display text-lg font-semibold text-[var(--text-primary)]">Assign Klien</h3>
                                            <p class="text-xs text-[var(--text-muted)] mt-0.5">Untuk {{ $tm->name }}</p>
                                        </div>
                            ...
                            <template x-teleport="body">
                                <div x-show="confirmDeactivate === {{ $user->id }}" x-cloak
                                    ...
                                    <div x-show="confirmDeactivate === {{ $user->id }}" x-transition
                                        ...
                                        <div class="flex items-center justify-between px-6 py-5 border-b border-[var(--border)]">
                                            <div>
                                                <h3 id="deactivate-user-modal-title-{{ $user->id }}" class="font-display text-lg font-semibold text-[var(--text-primary)]">Nonaktifkan Akun</h3>
                                                <p class="text-xs text-[var(--text-muted)] mt-0.5">{{ $tm->name }} ({{ $tm->operational_role ?? '-' }})</p>
                                            </div>
                                        ...
                                        <form action="{{ route('user-management.destroy', $user) }}" method="POST">
                                            ...
                                            <div class="px-6 py-5 space-y-4">
                                                <p class="text-sm text-[var(--text-secondary)]">Yakin ingin menonaktifkan akun sistem <strong class="text-[var(--text-primary)]">{{ $tm->name }}</strong>? User tidak akan bisa login sampai diaktifkan kembali.</p>

                                                @if ($user->active_task_count > 0)
                                                    <div class="bg-[var(--warning-tint)] text-[var(--warning-text)] text-xs p-3 rounded-lg">
                                                        {{ $tm->name }} masih PIC di <strong>{{ $user->active_task_count }} konten aktif</strong>. Pilih pengganti supaya konten-konten itu tidak nyangkut ke akun yang sudah tidak bisa login.
                                                    </div>
                                                    <div>
                            ...
                            <template x-teleport="body">
                                <div x-show="confirmActivate === {{ $user->id }}" x-cloak
                                    ...
                                    <div x-show="confirmActivate === {{ $user->id }}" x-transition
                                        ...
                                        <div class="flex items-center justify-between px-6 py-5 border-b border-[var(--border)]">
                                            <div>
                                                <h3 id="activate-user-modal-title-{{ $user->id }}" class="font-display text-lg font-semibold text-[var(--text-primary)]">Aktifkan Akun</h3>
                                                <p class="text-xs text-[var(--text-muted)] mt-0.5">{{ $tm->name }} ({{ $tm->operational_role ?? '-' }})</p>
                                            </div>
                                        ...
                                        <div class="px-6 py-5">
                                            <p class="text-sm text-[var(--text-secondary)]">Aktifkan kembali akun <strong class="text-[var(--text-primary)]">{{ $tm->name }}</strong>? User akan bisa login kembali seperti biasa.</p>
                                        </div>
    ...
    <div class="sm:hidden space-y-3">
        @forelse ($teamMembers as $tm)
            @php
                $tmClientsMobile = $tm->clients->pluck('name');
                $tmClientsMobileLabel = $tmClientsMobile->isEmpty() ? '-' : $tmClientsMobile->take(2)->join(', ').($tmClientsMobile->count() > 2 ? ', +'.($tmClientsMobile->count() - 2).' lainnya' : '');
                $userMobile = $tm->user;
            @endphp
            <div class="card p-3.5" x-data="{ open: false }">
                    ...
                    <div class="flex items-center gap-3 min-w-0">
                        <div class="w-9 h-9 rounded-full bg-[var(--brand-solid)] text-white flex items-center justify-center text-sm font-semibold shrink-0 overflow-hidden">
                            @if ($userMobile?->avatar_url)
                                <img src="{{ $userMobile->avatar_url }}" alt="" referrerpolicy="no-referrer" class="w-full h-full object-cover">
                            @else
                                {{ strtoupper(substr($tm->name, 0, 1)) }}
                            @endif
                        </div>
                        <div class="min-w-0">
                            <p class="font-medium text-[var(--text-primary)] truncate">{{ $tm->name }}</p>
                            <div class="flex items-center gap-2 mt-1">
                                <span class="text-xs text-[var(--text-secondary)]">{{ $tm->operational_role ?? '-' }}</span>
                                @if ($tm->hasAccount())
                                    <span class="badge badge-success">Akun aktif</span>
                                @else
                                    <span class="badge badge-neutral">Belum memiliki akun</span>
                ...
                <div x-show="open" x-cloak x-transition class="mt-3 pt-3 border-t border-[var(--surface-muted)] space-y-2">
                    <div class="flex items-center justify-between text-xs">
                        ...
                        <span class="text-[var(--text-primary)] font-medium truncate ml-3">{{ $tm->email ?? '-' }}</span>
                    </div>

                    <div class="text-xs">
                        <span class="text-[var(--text-muted)] block mb-1.5">Klien Ditangani</span>
                        <span class="text-[var(--text-secondary)]">{{ $tmClientsMobileLabel }}</span>
                    </div>
                    ...
                    <div class="flex items-center gap-2 pt-1">
                        ...
                        @if ($userMobile)
                            @if ($userMobile->status === 'inactive')
                                <button type="button" @click="confirmActivate = {{ $userMobile->id }}"
                                        class="w-8 h-8 flex items-center justify-center rounded-lg text-[var(--text-muted)] hover:bg-[var(--success-tint)] hover:text-[var(--success-text)] transition-colors" title="Aktifkan">
                                    <span class="material-symbols-outlined text-[17px]">restart_alt</span>
                                </button>
                            @else
                                <button type="button" @click="confirmDeactivate = {{ $userMobile->id }}"
                                        class="w-8 h-8 flex items-center justify-center rounded-lg text-[var(--text-muted)] hover:bg-[var(--danger-tint)] hover:text-[var(--danger-text)] transition-colors" title="Nonaktifkan">
                                    <span class="material-symbols-outlined text-[17px]">toggle_off</span>
                                </button>
                            @endif
