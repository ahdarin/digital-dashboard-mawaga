...
<div x-data="{ openAssign: null, confirmDeactivate: null, confirmActivate: null, showCreateModal: {{ $errors->any() ? 'true' : 'false' }} }" class="p-4 sm:p-6 lg:p-8 max-w-[1400px] mx-auto">
                    ...
                    <tr class="border-t border-[#f2f3f6] hover:bg-[#f7f8fc] transition-colors">
                        ...
                        <td class="px-6 py-3.5">
                            @if ($user->assignedClients->isEmpty())
                                <span class="text-xs text-[#767c80] italic">Belum ada client ditangani</span>
                            @else
                        ...
                        <td class="px-6 py-3.5">
                            <div class="flex items-center justify-end gap-1">
                                <button type="button" @click="openAssign = {{ $user->id }}"
                                        class="w-8 h-8 flex items-center justify-center rounded-lg text-[#767c80] hover:bg-[#f2f3f6] hover:text-[#044b46] transition-colors" title="Assign Klien">
                                    <span class="material-symbols-outlined text-[17px]">assignment_ind</span>
                                </button>

                                @if ($user->status === 'inactive')
                                    <button type="button" @click="confirmActivate = {{ $user->id }}"
                                            class="w-8 h-8 flex items-center justify-center rounded-lg text-[#767c80] hover:bg-[#f0f5f4] hover:text-[#0f7a5f] transition-colors" title="Aktifkan">
                                        <span class="material-symbols-outlined text-[17px]">restart_alt</span>
                                    </button>
                                @else
                                    <button type="button" @click="confirmDeactivate = {{ $user->id }}"
                                            class="w-8 h-8 flex items-center justify-center rounded-lg text-[#767c80] hover:bg-[#fdf2f1] hover:text-[#b3423e] transition-colors" title="Nonaktifkan">
                        ...
                        <div x-show="openAssign === {{ $user->id }}" x-cloak
                            ...
                            <div x-show="openAssign === {{ $user->id }}" x-transition
                                ...
                                <div class="flex items-center justify-between px-6 py-5 border-b border-[#eef0f4]">
                                    <div>
                                        <h3 id="assign-client-modal-title-{{ $user->id }}" class="font-display text-lg font-semibold text-[#14181a]">Assign Klien</h3>
                                        <p class="text-xs text-[#767c80] mt-0.5">Untuk {{ $user->name }} ({{ $user->role->name ?? '-' }})</p>
                                    </div>
                                    <button type="button" @click="openAssign = null" class="text-[#767c80] hover:text-[#5c6266]">
                                ...
                                <form action="{{ route('user-client-assignment.update', $user) }}" method="POST">
                                    @csrf @method('PUT')
                                    <div class="px-6 py-5">
                                        <p class="text-xs font-semibold text-[#767c80] uppercase mb-3">Pilih Client yang Ditangani</p>
                                            ...
                                            @forelse ($allClients as $client)
                                                ...
                                                <label class="flex items-center gap-3 p-3 border border-[#eef0f4] rounded-lg hover:bg-[#f7f8fc] cursor-pointer">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p class="text-xs text-[#767c80]">{{ $client->brand_name }}</p>
                                                    </div>
                                                </label>
                                            @empty
                                                <p class="text-xs text-[#767c80] italic">Belum ada client aktif.</p>
                                            @endforelse
                                        </div>
                                    </div>
                    ...
                    <template x-teleport="body">
                        <div x-show="confirmDeactivate === {{ $user->id }}" x-cloak
                            ...
                            <div x-show="confirmDeactivate === {{ $user->id }}" x-transition
                                ...
                                <div class="flex items-center justify-between px-6 py-5 border-b border-[#eef0f4]">
                                    <div>
                                        <h3 id="deactivate-user-modal-title-{{ $user->id }}" class="font-display text-lg font-semibold text-[#14181a]">Nonaktifkan User</h3>
                                        <p class="text-xs text-[#767c80] mt-0.5">{{ $user->name }} ({{ $user->role->name ?? '-' }})</p>
                                    </div>
                                    <button type="button" @click="confirmDeactivate = null" class="text-[#767c80] hover:text-[#5c6266]">
                                        <span class="material-symbols-outlined text-[19px]">close</span>
                                    </button>
                                </div>
                    ...
                    <template x-teleport="body">
                        <div x-show="confirmActivate === {{ $user->id }}" x-cloak
                            ...
                            <div x-show="confirmActivate === {{ $user->id }}" x-transition
                                ...
                                <div class="flex items-center justify-between px-6 py-5 border-b border-[#eef0f4]">
                                    <div>
                                        <h3 id="activate-user-modal-title-{{ $user->id }}" class="font-display text-lg font-semibold text-[#14181a]">Aktifkan User</h3>
                                        <p class="text-xs text-[#767c80] mt-0.5">{{ $user->name }} ({{ $user->role->name ?? '-' }})</p>
                                    </div>
                                    <button type="button" @click="confirmActivate = null" class="text-[#767c80] hover:text-[#5c6266]">
                                        <span class="material-symbols-outlined text-[19px]">close</span>
                                    </button>
                                </div>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center">
                            ...
                            <p class="text-sm text-[#767c80]">Belum ada user internal.</p>
        ...
        @forelse ($users as $user)
            <div class="card p-3.5" x-data="{ open: false }">
                ...
                <div x-show="open" x-cloak x-transition class="mt-3 pt-3 border-t border-[#f2f3f6] space-y-2">
                    ...
                    <div class="text-xs">
                        <span class="text-[#767c80] block mb-1.5">Client Ditangani</span>
                        @if ($user->assignedClients->isEmpty())
                            <span class="text-xs text-[#767c80] italic">Belum ada client ditangani</span>
                        @else
    ...
    <div x-show="showCreateModal" x-cloak x-transition
        ...
        <div x-show="showCreateModal" x-transition
             ...
             class="relative bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between px-6 py-5 border-b border-[#eef0f4]">
                <h3 id="invite-user-modal-title" class="font-display text-lg font-semibold text-[#14181a]">Undang User Baru</h3>
            ...
            <form action="{{ route('user-management.store') }}" method="POST">
                ...
                <div class="px-6 py-5 space-y-4">
                    <div>
                        <label for="invite_name" class="block text-xs font-medium text-[#767c80] uppercase mb-1.5">Nama</label>
                    ...
                    <div>
                        <label for="invite_email" class="block text-xs font-medium text-[#767c80] uppercase mb-1.5">Email Google</label>
                    ...
                    <div>
                        <label for="invite_role_id" class="block text-xs font-medium text-[#767c80] uppercase mb-1.5">Role</label>
                        <select id="invite_role_id" name="role_id" required class="w-full border border-[#eef0f4] rounded-lg px-3.5 py-2.5 text-sm bg-white focus:outline-none focus:border-[#044b46]/40">
