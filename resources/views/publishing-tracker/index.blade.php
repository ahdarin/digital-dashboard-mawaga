...
@section('content')
<div x-data="{ search: '', matches(...fields) { if (!this.search) return true; const s = this.search.toLowerCase(); return fields.some(f => f.toLowerCase().includes(s)); } }" class="p-8 max-w-[1400px]">
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center gap-2.5">
            <div class="relative">
                ...
                <input x-model="search" placeholder="Cari judul konten..."
                       class="pl-10 pr-4 h-[40px] bg-white border border-[#eef0f4] rounded-lg text-sm focus:outline-none focus:border-[#044b46]/40 w-64" type="text">
            </div>
            <form method="GET" class="flex items-center gap-2.5">
                <select name="platform_id" onchange="this.form.submit()"
                        class="border border-[#eef0f4] rounded-lg px-3.5 py-2 text-sm bg-white focus:outline-none focus:border-[#044b46]/40 h-[40px]">
                ...
                @forelse ($publications as $pub)
                    <tr x-show="matches('{{ addslashes($pub->contentItem->title) }}', '{{ addslashes($pub->contentItem->client->name ?? '') }}')"
                        ...
                        <td class="px-6 py-3.5 font-medium text-[#14181a]">
                            <a href="{{ route('content-items.show', $pub->contentItem) }}" class="hover:underline">{{ $pub->contentItem->title }}</a>
                        </td>
