...
<div class="mb-8">
    <h2 class="text-sm font-semibold text-[var(--text-primary)] mb-3">KPI Bulan Ini &middot; {{ $kpiPeriod->translatedFormat('F Y') }}</h2>
    ...
    @else
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
            <div class="card p-4 text-center bg-[var(--brand-tint)]">
                <p class="text-[10px] uppercase text-[var(--brand)] mb-1">Nilai KPI</p>
                <p class="font-display text-lg font-semibold text-[var(--brand)]">{{ round($kpiResult->final_score) }}</p>
                <span class="badge {{ $kpiStatusBadgeClass($kpiResult->final_score) }} mt-1">{{ $kpiResult->scoreLabel() }}</span>
            </div>
            <div class="card p-4 text-center">
                <p class="text-[10px] uppercase text-[var(--text-muted)] mb-1">Ketepatan Kerja</p>
                <p class="font-display text-lg font-semibold text-[var(--text-primary)]">{{ $kpiResult->timeliness_score !== null ? round($kpiResult->timeliness_score).'%' : '-' }}</p>
            </div>
            <div class="card p-4 text-center">
                <p class="text-[10px] uppercase text-[var(--text-muted)] mb-1">Kualitas Kerja</p>
                <p class="font-display text-lg font-semibold text-[var(--text-primary)]">{{ round($kpiResult->quality_score) }}%</p>
            </div>
            <div class="card p-4 text-center">
                <p class="text-[10px] uppercase text-[var(--text-muted)] mb-1">Bonus Analytics</p>
                <p class="font-display text-lg font-semibold text-[var(--text-primary)]">{{ $kpiResult->analytics_available ? '+'.round($kpiResult->analytics_bonus, 1) : 'Belum tersedia' }}</p>
            </div>
        </div>

        <div class="card p-6">
            <h3 class="text-sm font-semibold text-[var(--text-primary)] mb-4">Konten yang Menjadi Dasar Nilai ({{ $kpiResult->sample_size }})</h3>
                ...
                <table class="w-full text-xs">
                    <thead>
                        <tr class="text-[var(--text-muted)] text-left border-b border-[var(--surface-muted)]">
                            ...
                            <th class="py-2 font-medium text-right">Bonus Analytics</th>
                        </tr>
                    </thead>
                    <tbody>
