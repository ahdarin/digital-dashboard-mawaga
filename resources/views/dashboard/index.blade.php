...
@section('content')
    ...
    <div class="p-8 max-w-[1400px]">
        ...
        @php
            ...
            $greeting = match (true) {
                $greetingHour >= 4 && $greetingHour < 11 => 'Selamat pagi',
                $greetingHour >= 11 && $greetingHour < 15 => 'Selamat siang',
                $greetingHour >= 15 && $greetingHour < 18 => 'Selamat sore',
                default => 'Selamat malam',
            };

            // Ganti tiap hari (bukan tiap refresh) biar nggak norak diulang-ulang
            // tapi tetap kerasa "hidup" kalau dibuka besoknya lagi.
            $dailyReminders = [
                ['icon' => 'local_drink', 'text' => 'Jangan lupa minum air putih, biar tetap fokus seharian.'],
                ['icon' => 'self_improvement', 'text' => 'Coba stretching sebentar, duduk kelamaan bikin pundak protes.'],
                ['icon' => 'visibility', 'text' => 'Istirahatin mata tiap beberapa saat, layar nggak akan ke mana-mana.'],
                ['icon' => 'restaurant', 'text' => 'Sudah makan belum? Jangan sampai kelewat gara-gara keasyikan kerja.'],
                ['icon' => 'trending_up', 'text' => 'Progress kecil hari ini tetap progress, pelan-pelan aja nggak apa.'],
                ['icon' => 'bedtime', 'text' => 'Kalau mulai capek, boleh kok rehat sejenak sebelum lanjut lagi.'],
                ['icon' => 'favorite', 'text' => 'Semangat hari ini, tim yang sehat hasil kerjanya juga lebih oke.'],
            ];
            $dailyReminder = $dailyReminders[now()->dayOfYear % count($dailyReminders)];
        @endphp
        <div class="mb-6 rounded-2xl bg-[#044b46] px-7 py-5 flex items-center justify-between gap-5 flex-wrap">
            ...
            <div class="flex items-center gap-2.5 bg-white/10 rounded-full pl-3.5 pr-4 py-2 max-w-full">
                <span class="material-symbols-outlined text-white text-[18px] shrink-0">{{ $dailyReminder['icon'] }}</span>
                <p class="text-sm text-white/90">{{ $dailyReminder['text'] }}</p>
            </div>
        </div>
