...
<div class="p-8 max-w-6xl">
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="text-xs text-[#9aa0a4]">{{ $contentItem->client->name ?? '-' }} / Item #{{ $contentItem->id }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="lg:col-span-2 space-y-5">
            ...
            <div class="card p-5">
                ...
                <p class="text-sm text-[#5c6266] whitespace-pre-line mb-4">{{ $contentItem->brief ?: 'Belum ada brief.' }}</p>
            ...
            <div class="card p-5">
                <h3 class="text-sm font-semibold text-[#14181a] mb-4">Revision Log ({{ $contentItem->revisions->count() }})</h3>

                <div class="space-y-2.5 mb-4">
                    @forelse ($contentItem->revisions as $revision)
                        <div class="border border-[#eef0f4] rounded-lg p-3 {{ $revision->status === 'open' ? 'bg-[#fdf6ec]' : 'bg-[#f7f8fc]' }}">
                            <div class="flex items-center justify-between mb-1">
                                <p class="text-xs font-medium text-[#14181a]">Ronde {{ $revision->revision_round }} — {{ $revision->requestedBy->name ?? '-' }}</p>
                                <span class="text-[10px] px-2 py-0.5 rounded-full {{ $revision->status === 'open' ? 'bg-[#f7e8cf] text-[#8a6423]' : 'bg-[#f0f5f4] text-[#0f7a5f]' }}">{{ $revision->status }}</span>
                            </div>
                            ...
                            @if ($revision->status === 'open')
                                <form action="{{ route('content-revision.resolve', [$contentItem, $revision]) }}" method="POST" class="mt-2">
                                    @csrf @method('PATCH')
                                    <button class="text-[10px] font-medium text-[#044b46] hover:underline">Tandai Selesai</button>
                                </form>
                            @endif
                        </div>
                    @empty
                ...
                <form action="{{ route('content-revision.store', $contentItem) }}" method="POST" class="flex gap-2">
                    ...
                    <button type="submit" class="bg-[#044b46] text-white text-xs font-medium px-4 py-2 rounded-lg hover:bg-[#033b37]">Kirim</button>
                </form>
            </div>
                    ...
                    <form action="{{ route('content-publication.store', $contentItem) }}" method="POST" class="space-y-3">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <select name="platform_id" required class="w-full border border-[#eef0f4] rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-[#044b46]/40">
                            ...
                            <div>
                                ...
                                <input type="datetime-local" name="published_at" required class="w-full border border-[#eef0f4] rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-[#044b46]/40">
                            </div>
                        </div>
                        <div>
                            <label class="block text-[10px] font-medium text-[#9aa0a4] uppercase mb-1">Link Post</label>
                            <input type="url" name="post_url" placeholder="https://..." class="w-full border border-[#eef0f4] rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-[#044b46]/40">
                        </div>
                        <div>
                            <label class="block text-[10px] font-medium text-[#9aa0a4] uppercase mb-1">Caption Final</label>
                            <textarea name="caption_final" rows="2" class="w-full border border-[#eef0f4] rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-[#044b46]/40"></textarea>
                        </div>
                        <button type="submit" class="bg-[#044b46] text-white text-xs font-medium px-4 py-2 rounded-lg hover:bg-[#033b37]">Simpan &amp; Tandai Uploaded</button>
                    </form>
                </div>
            @else
                <div class="card p-5">
                    ...
                    @foreach ($contentItem->publications as $pub)
                        <div class="text-xs text-[#5c6266] border-b border-[#f2f3f6] py-2 last:border-0">
                            <p class="font-medium text-[#14181a]">{{ $pub->platform->name }} — {{ $pub->published_at->format('d M Y, H:i') }}</p>
                            @if ($pub->post_url)
                ...
                <div class="space-y-3">
                    @forelse ($contentItem->assignments as $assignment)
                        <div class="flex items-center gap-2">
                            ...
                            @else
                                <div class="w-8 h-8 rounded-full bg-[#044b46] text-white text-xs font-semibold flex items-center justify-center">{{ strtoupper(substr($assignment->user->name, 0, 1)) }}</div>
                            @endif
                            <div>
                        ...
                        <div class="flex gap-3">
                            ...
                            <div>
                                <p class="text-xs text-[#5c6266]">
                                    <span class="font-medium text-[#14181a]">{{ $log->from_status ? ($statusLabels[$log->from_status] ?? $log->from_status) : 'Dibuat' }}</span>
                                    →
                                    <span class="font-medium text-[#14181a]">{{ $statusLabels[$log->to_status] ?? $log->to_status }}</span>
                                </p>
                                <p class="text-[10px] text-[#9aa0a4]">{{ $log->changedBy->name ?? '-' }} · {{ $log->changed_at->format('d M, H:i') }}</p>
                                @if ($log->notes)
