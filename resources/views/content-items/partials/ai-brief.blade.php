...
@if (! $contentBrief)
    ...
    <div class="card p-6 border-2 border-[#044b46]/20 bg-gradient-to-br from-[var(--brand-tint)] to-white">
        <div class="flex items-start justify-between gap-4 flex-wrap">
            <div class="flex items-start gap-3">
                <div class="w-11 h-11 rounded-xl bg-[var(--brand)] text-white flex items-center justify-center shrink-0">
            ...
            <div x-data="{ editingTakeDate: {{ $contentItem->footage_captured_at ? 'false' : 'true' }} }" class="mb-4">
                ...
                <form x-show="editingTakeDate" action="{{ route('content-items.footage-captured', $contentItem) }}" method="POST"
                    ...
                    <input type="text" name="footage_captured_at" data-flatpickr="datetime" autocomplete="off"
                        ...
                        class="flex-1 min-w-0 border border-[var(--border)] rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#044b46]/40">
                    <button type="submit"
                    ...
                    @foreach ($scenesForDisplay as $i => $scene)
                        <div class="border border-[var(--border)] rounded-xl overflow-hidden">
                            <div class="flex items-center gap-2 bg-[var(--surface-page)] border-b border-[var(--border)] px-4 py-2">
                                <span class="w-5 h-5 rounded-full bg-[var(--brand)] text-white text-[10px] font-bold flex items-center justify-center shrink-0">{{ $i + 1 }}</span>
