...
@else
    ...
    <div class="card p-6 border border-[#044b46]/15" x-data="{ editing: false }">
        ...
        @if ($isVideo && $contentItem->workflow->current_status === 'in_progress')
            <div x-data="{ editingTakeDate: {{ $contentItem->footage_captured_at ? 'false' : 'true' }} }" class="mb-4">
                ...
                <form x-show="editingTakeDate" action="{{ route('content-items.footage-captured', $contentItem) }}" method="POST"
                    ...
                    @if ($contentItem->footage_captured_at)
                        <button type="button" @click="editingTakeDate = false" class="text-xs font-medium text-[#767c80] hover:text-[#14181a] px-2">Batal</button>
                    @endif
                </form>
            </div>
        @endif
                ...
                <form action="{{ route('content-brief.update', $contentBrief) }}" method="POST" class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="sm:col-span-2">
                            <label for="hook_title" class="block text-[10px] font-semibold text-[#767c80] uppercase mb-1.5">Hook / Judul Brief</label>
                        ...
                        <div>
                            <label for="talent" class="block text-[10px] font-semibold text-[#767c80] uppercase mb-1.5">Talent</label>
                            <input id="talent" type="text" name="talent" value="{{ $contentBrief->talent }}"
                                class="w-full border border-[#eef0f4] rounded-lg px-3.5 py-2.5 text-sm bg-white focus:outline-none focus:border-[#044b46]/40">
                        </div>
                        <div>
                            <label for="properti" class="block text-[10px] font-semibold text-[#767c80] uppercase mb-1.5">Properti</label>
                            <input id="properti" type="text" name="properti" value="{{ $contentBrief->properti }}"
                                class="w-full border border-[#eef0f4] rounded-lg px-3.5 py-2.5 text-sm bg-white focus:outline-none focus:border-[#044b46]/40">
                        </div>
                        <div>
                            <label for="platform" class="block text-[10px] font-semibold text-[#767c80] uppercase mb-1.5">Platform</label>
                    ...
                    <div x-data="{
                        ...
                        <div class="flex items-center justify-between mb-2.5">
                            <label class="block text-[10px] font-semibold text-[#767c80] uppercase">Adegan / Slide</label>
                                ...
                                <div class="border border-[#eef0f4] rounded-xl bg-white overflow-hidden">
                                    <div class="flex items-center justify-between gap-2 bg-[#f7f8fc] border-b border-[#eef0f4] px-3.5 py-2">
                                        <input type="text" :name="'scenes[' + index + '][label]'" x-model="scene.label"
                                            ...
                                            class="flex-1 text-xs font-semibold text-[#14181a] bg-transparent border-0 focus:outline-none focus:ring-0 p-0 uppercase placeholder:text-[#767c80] placeholder:normal-case">
                                    ...
                                    <div class="divide-y divide-[#eef0f4]">
                                        <div class="p-3.5">
                                            <label :for="'scene_' + index + '_visual'" class="flex items-center gap-1.5 text-[10px] font-semibold text-[#767c80] uppercase mb-1.5">
                                        ...
                                        <div class="p-3.5">
                                            <label :for="'scene_' + index + '_talent_script'" class="flex items-center gap-1.5 text-[10px] font-semibold text-[#767c80] uppercase mb-1.5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 pb-4 mb-4 border-b border-[#f2f3f6]">
            <div>
                <p class="flex items-center gap-1 text-[10px] text-[#767c80] uppercase font-semibold mb-1">
                ...
                <div class="space-y-3">
                    @foreach ($scenesForDisplay as $i => $scene)
                        <div class="border border-[#eef0f4] rounded-xl overflow-hidden">
                            ...
                            <div class="divide-y divide-[#eef0f4]">
                                <div class="px-4 py-3">
                                    <p class="flex items-center gap-1.5 text-[10px] font-semibold text-[#767c80] uppercase mb-1.5">
                                        <span class="material-symbols-outlined text-[13px]">visibility</span> Visual
                                    </p>
        ...
        <div class="card p-5 bg-[#eef2fb] border-0 mb-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                @if ($contentBrief->estimated_duration_seconds)
                    <div><span class="text-[#767c80]">Durasi:</span> {{ $contentBrief->estimated_duration_seconds }} detik</div>
                @endif
                @if ($contentBrief->slide_count)
                    <div><span class="text-[#767c80]">Jumlah slide:</span> {{ $contentBrief->slide_count }}</div>
                @endif
                <div><span class="text-[#767c80]">Talent:</span> {{ $contentBrief->talent_count ?? 0 }} orang</div>
                <div><span class="text-[#767c80]">Lokasi:</span> {{ $contentBrief->location_count ?? 0 }} lokasi</div>
            </div>
        ...
        @if (! $contentBrief->isLocked() && auth()->user()->hasPermissionTo('content_plan', 'create'))
            <div class="flex items-start gap-2 mb-3 px-3.5 py-3 rounded-lg bg-[#f7f8fc] border border-[#eef0f4]">
                <span class="material-symbols-outlined text-[15px] text-[#767c80] mt-0.5">handshake</span>
            ...
            @if ($contentBrief->canRevert())
                ...
                <p class="text-[11px] text-[#767c80] text-center mt-1.5">Membatalkan perubahan terakhir (regenerate/edit/apply), balik ke isi sebelumnya.</p>
            @endif
